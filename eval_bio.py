...
from Res_Unet import unet_resnet_2d
	...
	def generate_result_4channel(self):
		if 1:
			if 1:
				...
				data_path = '/media/data/fanxuan/data/PART2_h5data/data_wave_50_bior3_7.h5'
				data_path_normal = '/media/data/fanxuan/data/PART2_h5data/data_50_new.h5'
				save_dir = '/media/data/fanxuan/result/FCN_50_resnet_bior3_7'
				model_path = os.path.join(save_dir, 'model/generator_epoch_200.hdf5') 
				save_path = os.path.join(save_dir, 'valid_200')
				...
				with tf.Session(graph=tf.get_default_graph(), config=config) as sess:
					...
					K.set_session(sess)
					generator = unet_resnet_2d((4,176,176))
					...
					for patient in patients:
						...
						for i in range(len(filenames)):
							if patient==filenames[i].split('+')[1]:
								...
								x_re = pywt.idwt2(coeffs_x, 'bior3.7')

								x_re = np.concatenate((x_normal[0:(x_normal.shape[0]//2-169), (x_normal.shape[1]//2-169):(x_normal.shape[1]//2+169)],x_re,x_normal[(x_normal.shape[0]//2+169):, (x_normal.shape[1]//2-169):(x_normal.shape[1]//2+169)]), axis=0)
								print(x_re.shape)
								x_re = np.concatenate((x_normal[:, :(x_normal.shape[1]//2-169)],x_re,x_normal[:, (x_normal.shape[1]//2+169):]), axis = 1)
								print(x_re.shape)
								x_volume[:,:,s] += x_re * max_img
								coeffs_gd = gd_slice[0], (gd_slice[1], gd_slice[2], gd_slice[3])
								gd_re = pywt.idwt2(coeffs_gd, 'bior3.7')
								gd_re = np.concatenate((gd_normal[0:(gd_normal.shape[0]//2-169), (gd_normal.shape[1]//2-169):(gd_normal.shape[1]//2+169)],gd_re,gd_normal[(gd_normal.shape[0]//2+169):, (gd_normal.shape[1]//2-169):(gd_normal.shape[1]//2+169)]), axis=0)
								print(gd_re.shape)
								gd_re = np.concatenate((gd_normal[:, :(gd_normal.shape[1]//2-169)],gd_re,gd_normal[:, (gd_normal.shape[1]//2+169):]), axis = 1)                                              
								...
								out_re = pywt.idwt2(coeffs_out, 'bior3.7')
								out_re = np.concatenate((x_normal[0:(x_normal.shape[0]//2-169), (x_normal.shape[1]//2-169):(x_normal.shape[1]//2+169)],out_re,x_normal[(x_normal.shape[0]//2+169):, (x_normal.shape[1]//2-169):(x_normal.shape[1]//2+169)]), axis=0)
								print(out_re.shape)
								out_re = np.concatenate((x_normal[:, :(x_normal.shape[1]//2-169)],out_re,x_normal[:, (x_normal.shape[1]//2+169):]), axis = 1)                                                
