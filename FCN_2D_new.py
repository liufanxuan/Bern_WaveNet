...
class SR_UnetGAN():
    def __init__(self):
        ...
        self.save_dir = '/media/data/fanxuan/result/FCN_10_50_wave_new'
        ...
        with tf.Session(graph=tf.get_default_graph(), config=config) as sess:
            ...
            for epoch in range(1, self.epochs+1):
                ...
                # number_of_batches
                    ...
                    print(x_con.shape)
                    g_loss2 = self.generator2.train_on_batch(x_con, y_batch)
                    gen_losses2.append(g_loss2)
                    print("    G_loss:{}, {}\n".format(g_loss1,g_loss2))
                ...
                # int(len(valid_filenames) / self.batch_size)
                    ...
                    a2 = np.mean(np.square(np.array(y_val) - np.array(pred2_val)), axis=-1)
                    print(a.shape)
                    val_loss1 = np.mean(a)
                    val_loss2 = np.mean(a2)
                    print("    val_loss: {},{}\n".format(val_loss1,val_loss2))
                ...
                batch_df = batch_df[['epoch', 'batch', 'generator_loss1','generator_loss2']]
                epoch_df = epoch_df[['epoch', 'generator_loss1','generator_loss2']]
                ...
                batch_val = batch_val[['epoch', 'batch', 'val_loss1','val_loss2']]
                epoch_val = epoch_val[['epoch', 'val_loss1','val_loss2']]
