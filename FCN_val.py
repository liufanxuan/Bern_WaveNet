...
random.seed(2)
def test():
    model = unet_model_3d((8,176,176,16))
    model.load_weights('/media/data/fanxuan/result/FCN_multi_3/model/generator_epoch_4.hdf5')
class SR_UnetGAN():
    def __init__(self):
        self.data_path = '/media/data/fanxuan/data/data_h5data/data_all.h5'
        #data_Siemens_wave_50_bior3_7_3d.h5
        self.save_dir = '/media/data/fanxuan/result/FCN_multi_3_new'
        self.model_path = '/media/data/fanxuan/result/FCN_multi_3/model/generator_epoch_4.hdf5'
        ...
        self.generator = self.build_generator(self.model_path)
    ...
    def train(self):
        ...
        if 1: 
            ...
            if 1:                    
                ...
                batch_val = batch_val.append(pd.DataFrame({'epoch': [1],
                                                         'batch': np.arange(1, len(val_losses)+1),
                                                         'val_loss': val_losses}))
                epoch_val = epoch_val.append(pd.DataFrame({'epoch': [1],
