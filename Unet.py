...
from keras.layers import Conv2D, MaxPooling2D, Add, UpSampling2D, Activation, BatchNormalization, PReLU, Deconvolution2D
...
def unet_model_2d(input_shape, final_act=None, pool_size=(2, 2), n_labels=1, initial_learning_rate=0.00001, deconvolution=False,
    ...
    return model
def unet_model_sig_2d(input_shape, final_act="sigmoid", pool_size=(2, 2), n_labels=1, initial_learning_rate=0.00001, deconvolution=False,
                  depth=4, n_base_filters=32, batch_normalization=False, activation_name="sigmoid"):
    ...
    if final_act:
        act = Activation(final_act)(final_convolution)
        act = layers.multiply([act,final_convolution])
        model = Model(inputs=inputs, outputs=act)
    else:
        model = Model(inputs=inputs, outputs=final_convolution)
...
def unet_res_2d(input_shape, final_act=None, pool_size=(2, 2), n_labels=1, initial_learning_rate=0.00001, deconvolution=False,
                  depth=4, n_base_filters=32, batch_normalization=False, activation_name="sigmoid"):
    ...
    # add levels with max pooling
    for layer_depth in range(depth):
        res = Conv2D(n_t*n_base_filters*(2**layer_depth)*2, kernel_size=1, padding='same', strides=strides)(current_layer)
        layer1 = create_convolution_block(input_layer=current_layer, n_filters=n_t*n_base_filters*(2**layer_depth),
                                          batch_normalization=batch_normalization)
        cov2 = Conv2D(n_t*n_base_filters*(2**layer_depth)*2, kernel_size=3, padding='same', strides=strides)(layer1)
    ...
    for layer_depth in range(depth-2, -1, -1):
        up_convolution = get_up_convolution(pool_size=pool_size, deconvolution=deconvolution,
                                            n_filters=current_layer._keras_shape[1])(current_layer)
        concat = concatenate([up_convolution, levels[layer_depth][1]], axis=1)
        res = Conv2D(levels[layer_depth][1]._keras_shape[1], kernel_size=1, padding='same', strides=strides)(concat)
        current_layer = create_convolution_block(n_filters=levels[layer_depth][1]._keras_shape[1],
                                                 input_layer=concat, batch_normalization=batch_normalization)
        up_conv2 = Conv2D(levels[layer_depth][1]._keras_shape[1], kernel_size=3, padding='same', strides=strides)(current_layer)
        if batch_normalization:
            up_conv2 = BatchNormalization(axis=1)(up_conv2)
        resconnection = Add(name="upAdd{}_1".format(layer_depth))([res, up_conv2])
