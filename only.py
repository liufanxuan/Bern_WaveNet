...
class SR_UnetGAN():
    def __init__(self):
        ...
        self.save_dir = '/media/data/fanxuan/result/FCN_multi_test_result'
        ...
        #self.generator_parallel = multi_gpu_model(self.generator,1)
    ...
    def train(self):
        ...
        index_all2 = np.arange(1,len(test_filenames))
        ...
        with tf.Session(graph=tf.get_default_graph(), config=config) as sess:
            ...
            self.generator.load_weights('/media/data/fanxuan/result/FCN_multi_3_10/model/generator_epoch_118.hdf5')
            # Add a loop, which will run for a specified number of epochs:
            for epoch in range(1, self.epochs+1):
                # Create two lists to store losses
                val_losses = []
                ori_losses = []
                for ind in range(int(len(test_filenames) / self.batch_size / 10)):
                ...
                for index in trange(number_of_batches//6):
                # number_of_batches
