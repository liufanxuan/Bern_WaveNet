...
class Evaluate():
	...
	def generate_all_result_3d(self):
		if 1:
			if 1:
				data_path = '/media/data/fanxuan/data/PART2_h5data/data_wave_10_3d_16.h5'
				data_path_reduce = '/media/data/fanxuan/data/PART2/10_dose'
				data_path_normal = '/media/data/fanxuan/data/PART2/Normal'
				save_dir = '/media/data/fanxuan/result/FCN_10_wave_se_3d'
				save_path = os.path.join(save_dir, 'valid_all')
				start_num = 15
				end_num = 40
				if not os.path.exists(save_path):
					os.makedirs(save_path)
				model_num_list,average_mse_list,average_nrmse_list,average_mape_list,average_psnr_list,average_ssim_list = [],[],[],[],[],[]                      
				for model_index in range(start_num,end_num+1):
					...
					with tf.Session(graph=tf.get_default_graph(), config=config) as sess:
						...
						K.set_session(sess)
						generator = unet_se_3d((8,176,176,16))
						...
						average_nrmse_list.append(np.mean(nrmse_list))
						average_mape_list.append(np.mean(mape_list))
						average_psnr_list.append(np.mean(psnr_list))
						average_ssim_list.append(np.mean(ssim_list))	
						data.close()			                  
				df_all = pd.DataFrame({ 'Model_num': model_num_list, 'MSE': average_mse_list,'NRMSE %': average_nrmse_list,'mape': average_mape_list, 'PSNR': average_psnr_list, 'SSIM': average_ssim_list})
				df_all = df_all[['Model_num', 'MSE', 'NRMSE %','mape', 'PSNR', 'SSIM']]
				df_all.to_csv(os.path.join(patient_save_path,'all.csv'), index=False)
	...
	def generate_result_3d(self):
		if 1:
			if 1:
				...
				data_path_reduce = '/media/data/fanxuan/data/PART2/10_dose'
				data_path_normal = '/media/data/fanxuan/data/PART2/Normal'
				save_dir = '/media/data/fanxuan/result/FCN_10_wave_DWT_3d_2'
