...
class SR_UnetGAN():
    def __init__(self):
        ...
        self.test_data_path = '/media/data/fanxuan/data/data_h5data/data_test_2.h5'
        #data_Siemens_wave_50_bior3_7_3d.h5
        self.save_dir = '/media/data/fanxuan/result/FCN_multi_test2'
    ...
    def train(self):
        ...
        test_len = int(len(test_filenames) / 4)
        index_all2 = np.arange(1,test_len*4)
        ...
        with tf.Session(graph=tf.get_default_graph(), config=config) as sess:
            ...
            for num in range(100):
                model_num = 197-num
                model_path = os.path.join('/media/data/fanxuan/result/FCN_multi_3_10/model','generator_epoch_'+str(model_num)+'.hdf5')
            ...
            # Add a loop, which will run for a specified number of epochs:
                ...
                epoch_val = epoch_val.append(pd.DataFrame({'num': [model_num],
                                                         'val_loss': [np.mean(val_losses)]}))
                epoch_val = epoch_val[['num', 'val_loss']]
                epoch_val.to_csv(os.path.join(log_dir, 'epoch_val_loss.csv'), index=False)


                epoch_ori = epoch_ori.append(pd.DataFrame({'num': [model_num],
                                                         'ori_loss': [np.mean(ori_losses)]}))
                epoch_ori = epoch_ori[['num', 'ori_loss']]
